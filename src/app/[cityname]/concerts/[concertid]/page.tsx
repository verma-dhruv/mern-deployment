"use client"
import React from 'react'
import { BsShare } from 'react-icons/bs'
import { BsFillStarFill } from 'react-icons/bs';
import './ConcertPage.css'
import ConcertCarousel from '@/components/concertcarousel/ConcertCarousel';


import 'swiper/css';
import 'swiper/css/pagination';
import { Swiper, SwiperSlide } from 'swiper/react';
import { Pagination } from 'swiper/modules';
import CelebCard from '@/components/CelebCard/CelebCard';
import { usePathname, useParams } from 'next/navigation'
import Link from 'next/link';


const ConcertPage = () => {
    const pathname = usePathname()
    const { concertid } = useParams()
   
    const [concert, setConcert] = React.useState<any>(null)
    console.log(concertid)

    const getConcert = async () => {
        fetch(`${process.env.NEXT_PUBLIC_BACKEND_API}/concert/concerts/${concertid}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
            credentials: 'include'
        })
            .then((res) => res.json())
            .then((data) => {
                if (data.ok) {
                    console.log(data)
                    setConcert(data.data)
                }
            })
            .catch((err) => {
                console.log(err)
            })
    }
   

    React.useEffect(() => {
        getConcert()
    }, [])

    return (
        <>
            {
                concert &&
                <div className='concertpage'>
                    <div className='c1' style={{
                        backgroundImage: `url(${concert.landscapeImgUrl})`
                    }}>
                        <div className='c11'>
                            <div className='left'>
                                <div className='concert_poster'
                                    style={{
                                        backgroundImage: `url(${concert.portraitImgUrl})`
                                    }}
                                >
                                    <p>In cinemas</p>
                                </div>
                                <div className='concert_details'>
                                    <p className='title'>
                                        {concert.title}
                                    </p>
                                    <p className='rating'>
                                        <BsFillStarFill className='star' />&nbsp;&nbsp;
                                        {concert.rating}/10
                                    </p>
                                    {}
                                    <p className='duration_type_releasedat'>
                                        <span className='duration'>
                                            {concert.duration}
                                        </span>
                                        <span>•</span>
                                        <span className='type'>
                                            {concert.genre.join(', ')}
                                        </span>
                                        {}
                                    </p>
                                    <Link
                                        href={`${pathname}/buytickets`}
                                        className='linkstylenone'
                                    >
                                        <button className='bookbtn'>Book Tickets</button>
                                    </Link>

                                </div>
                            </div>
                            <div className='right'>

                                <button className='sharebtn'><BsShare className='shareicon' />Share</button>
                            </div>
                        </div>
                    </div>
                    <div className='c2'>
                        <h1>About the Concert</h1>
                        <p>{concert.description}</p>
                        {
                            concert.cast.length>0 &&
                            <div className='circlecardslider'>
                                <div className='line'></div>

                                <h1>Cast</h1>
                                <Swiper
                                    slidesPerView={1}
                                    spaceBetween={1}
                                    pagination={{
                                        clickable: true,
                                    }}
                                    breakpoints={{
                                        '@0.00': {
                                            slidesPerView: 1,
                                            spaceBetween: 2,
                                        },
                                        '@0.75': {
                                            slidesPerView: 2,
                                            spaceBetween: 2,
                                        },
                                        '@1.00': {
                                            slidesPerView: 3,
                                            spaceBetween: 2,
                                        },
                                        '@1.50': {
                                            slidesPerView: 6,
                                            spaceBetween: 2,
                                        },
                                    }}
                                    modules={[Pagination]}
                                    className="mySwiper"
                                >
                                    {
                                        concert.cast.map((cast, index) => {
                                            return (
                                                <SwiperSlide key={index}>
                                                    <CelebCard {...cast} />
                                                </SwiperSlide>
                                            )
                                        })
                                    }
                                </Swiper>
                            </div>
                        }
                        {
                            concert.crew.length>0 &&
                            <div className='circlecardslider'>
                                <div className='line'></div>

                                <h1>Crew</h1>
                                <Swiper
                                    slidesPerView={1}
                                    spaceBetween={1}
                                    pagination={{
                                        clickable: true,
                                    }}
                                    breakpoints={{
                                        '@0.00': {
                                            slidesPerView: 1,
                                            spaceBetween: 2,
                                        },
                                        '@0.75': {
                                            slidesPerView: 2,
                                            spaceBetween: 2,
                                        },
                                        '@1.00': {
                                            slidesPerView: 3,
                                            spaceBetween: 2,
                                        },
                                        '@1.50': {
                                            slidesPerView: 6,
                                            spaceBetween: 2,
                                        },
                                    }}
                                    modules={[Pagination]}
                                    className="mySwiper"
                                >
                                    {
                                        concert.crew.map((cast, index) => {
                                            return (
                                                <SwiperSlide key={index}>
                                                    <CelebCard {...cast} />
                                                </SwiperSlide>
                                            )
                                        })
                                    }
                                </Swiper>
                            </div>
                        }
                        <div className='line'></div>
                        <h1>Your might also like</h1>
                        <ConcertCarousel />
                    </div>

                </div>
            }
        </>
    )
}

export default ConcertPage